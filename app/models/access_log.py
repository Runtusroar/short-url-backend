...
class AccessLog(Base):
    ...
    short_link_id = Column(
        UUID(as_uuid=True),
        ForeignKey(
            "short_links.id", name="fk_access_logs_short_link", ondelete="RESTRICT"
        ),
        nullable=False,
    )
    domain_id = Column(
        UUID(as_uuid=True),
        ForeignKey("domains.id", name="fk_access_logs_domain", ondelete="RESTRICT"),
        nullable=False,
    )
    target_url_id = Column(
        UUID(as_uuid=True),
        ForeignKey(
            "target_urls.id", name="fk_access_logs_target_url", ondelete="SET NULL"
        ),
        nullable=True,
    )
    matched_rule_id = Column(
        UUID(as_uuid=True),
        ForeignKey(
            "access_rules.id", name="fk_access_logs_matched_rule", ondelete="SET NULL"
        ),
        nullable=True,
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "result = ANY (ARRAY['allowed', 'denied', 'blocked'])",
            name="ck_access_logs_result",
        ),
        CheckConstraint(
            "country IS NULL OR country ~ '^[A-Z]{2}$'", name="ck_access_logs_country"
        ),
        CheckConstraint(
            "decision_reason = ANY (ARRAY['legacy_unknown', 'blacklist', "
            "'matched_rule', 'default_action', 'no_target'])",
            name="ck_access_logs_decision_reason",
        ),
        CheckConstraint(
            "request_method = ANY (ARRAY['GET', 'HEAD'])",
            name="ck_access_logs_request_method",
        ),
        CheckConstraint(
            "proxy_check_status = ANY (ARRAY['skipped', 'cached', 'checked', "
            "'assumed_bot', 'error'])",
            name="ck_access_logs_proxy_check_status",
        ),
        CheckConstraint(
